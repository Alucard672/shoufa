...
function parseExpireDateToDate(expireDate) {
  ...
  if (typeof expireDate === 'string') {
    ...
    // 'YYYY-MM-DD'：按当天结束（使用本地时区，避免时区偏移）
    // 使用本地时区解析，确保日期不会因为时区转换而偏移
    const parts = s.split('-')
    if (parts.length === 3) {
      ...
      if (!isNaN(year) && !isNaN(month) && !isNaN(day)) {
        // 创建本地时间的 Date 对象，设置为当天的 23:59:59.999
...
export function calculateRemainingDays(expireDate) {
  ...
  // 使用日期（年月日）进行比较，而不是时间戳，避免时区和时间精度问题
  // 将两个日期都设置为当天的 00:00:00，然后计算日期差
  const nowDate = new Date(now.getFullYear(), now.getMonth(), now.getDate())
  const expireDateOnly = new Date(expire.getFullYear(), expire.getMonth(), expire.getDate())
  
  const diff = expireDateOnly.getTime() - nowDate.getTime()
